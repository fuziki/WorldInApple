...
public class WorldInAppleSynthesizer3: WorldInAppleComponents {
    ...
    public func synthesis(buffer: AVAudioPCMBuffer) -> AVAudioPCMBuffer? {
        ...
        InitializeSynthesizer(Int32(parameters.fs), parameters.frame_period,
                              Int32(parameters.fft_size), Int32(buffer_size), 100, &worldSynthesizer)

        var offset = 0
        var index = 0

        for i in 0..<parameters.f0_length {
            let res = AddParameters(parameters.f0.advanced(by: i), 1, parameters.spectrogram.advanced(by: i),
                                    parameters.aperiodicity.advanced(by: i), &worldSynthesizer)
            if res == 0 {
